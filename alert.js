...
async function runScanMode() {
  ...
  if (openTrade) {
    ...
    if (!openTrade.tp1Reached) {
      ...
      if (tp1Hit) {
        openTrade.tp1Reached = true; openTrade.macdEarlyFlipEpoch = null;
        fs.writeFileSync("trades.json", JSON.stringify(trades, null, 2));
    ...
    if (pnl >= TRAIL_ACTIVATE_USD) {
      ...
      if (openTrade.peakProfit !== null && pnl < openTrade.peakProfit - TRAIL_DROP_USD) {
        ...
        await closeWith(result, `📉 High-water trail exit: ${result} | ${openTrade.direction} | Peak: $${openTrade.peakProfit.toFixed(2)} | Now: $${pnl.toFixed(2)}`); return;
      }
    }
    ...
    if (!openTrade.tp1Reached) {
      ...
      if (m5Early && m5Early.length >= 52) {
        ...
        const sf = sma(cls, 2), ss = sma(cls, 50), ci = m5Early.length - 2;
    ...
    if (openTrade.tp1Reached) {
      ...
      if (m5c && m5c.length >= 100) {
        ...
        const macdFast = ema(cls, 8), macdSlow = ema(cls, 100), ci = m5c.length - 2;
